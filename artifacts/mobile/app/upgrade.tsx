...
export default function UpgradeScreen() {
  ...
  async function handleSelectPlan(plan: Plan) {
    ...
    try {
      ...
      if (!API_URL) {
        // Fallback: directly set premium via Clerk metadata (no Stripe)
        await user.update({
          unsafeMetadata: { isPremium: true, accountType: plan.accountType },
        });
        Alert.alert(
          "Welcome to Premium!",
      ...
      // Fetch products to find the right price
      ...
      // Find matching product by accountType metadata
      ...
      if (!product || !product.prices?.[0]) {
        // Stripe not configured yet — use direct upgrade
        await user.update({
          unsafeMetadata: { isPremium: true, accountType: plan.accountType },
        });
        Alert.alert(
    ...
    try {
      if (pendingSessionId && API_URL) {
        const verifyRes = await fetch(
          `${API_URL}/api/checkout-verify?sessionId=${pendingSessionId}&userId=${user.id}&accountType=${pendingPlan.accountType}`
        );
        ...
        if (verifyRes.ok) {
          ...
          if (success) {
            await user.update({
              unsafeMetadata: {
                isPremium: true,
                accountType: pendingPlan.accountType,
              },
            });
            Alert.alert(
              "Payment confirmed!",
        ...
        // No session ID — check subscription from API
        const subRes = await fetch(
          `${API_URL}/api/subscription?userId=${user.id}`
        );
        if (subRes.ok) {
          const { isPremium, accountType } = await subRes.json();
          if (isPremium) {
            await user.update({
              unsafeMetadata: { isPremium: true, accountType },
            });
        ...
        Alert.alert("Not yet confirmed", "Your payment hasn't been confirmed yet. Please wait a moment and try again.");
      }
    } catch {
          ...
          <View style={s.verifyCard}>
            ...
            <View style={s.verifyContent}>
              ...
              <Text style={s.verifyDesc}>
                After completing your payment in the browser, tap below to activate your premium account.
              </Text>
              <Pressable
